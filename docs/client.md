...
Metody:
   ...
   * public **ArrayList<SchoolClass>** getAllClass() throws **ClientError**, **JsonProcessingException** - pobiera plan zajęć klasy o podanym numerze id `classId` za pomocą żądanie HTTP (`/timetable/:classId/` GET), metoda może rzucić wyjątek **JsonProcessingException** podczas przetwarzania odpowiedzi do obiektu **ArrayList<SchoolClass>**, ***należy pamiętać że endpoint `/timetable/:classId/` jest dostępny tylko dla zalogowanych użytkowników z uprawnieniami administratora!!!***

   * public **void** addClassSchedule() throws **ClientError**, **JsonProcessingException** - Dodaje zajęcia szkolne do planu zajęć dla wybranej klasy za pomocą żądania HTTP (`/timetable/` POST), metoda może rzucić wyjątek **JsonProcessingException** podczas przetwarzania obiektu **SchoolClass** do JSON, ***należy pamiętać że endpoint `/timetable/` jest dostępny tylko dla zalogowanych użytkowników z uprawnieniami administratora!!!***
