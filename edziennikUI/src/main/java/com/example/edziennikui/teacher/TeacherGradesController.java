...
public class TeacherGradesController {
    ...
    @FXML
    private TableView<StudentGradesRow> gradesTable;
    @FXML
    private TableColumn<StudentGradesRow, String> colStudentName;
    @FXML
    private TableColumn<StudentGradesRow, String> colGrades;
    @FXML
    private TableColumn<StudentGradesRow, Double> colAverage;
    @FXML
    private TableColumn<StudentGradesRow, Void> colAction;
    ...
    private void setupTableColumns() {
        ...
        colGrades.setCellValueFactory(data -> {
            String gradesStr = data.getValue().grades.stream()
                    .map(g -> g.getGrade().toString())
                    .collect(Collectors.joining(", "));
            return new SimpleStringProperty(gradesStr);
        });

        colAverage.setCellValueFactory(data -> {
            double avg = data.getValue().grades.stream()
                    .map(Grade::getGrade)
    ...
    private void handleRefresh() {
        ...
        try {
            ...
            for (Student s : students) {
                ArrayList<Grade> grades = ApplicationClient.getInstance().getGradesList(s.getId());
                List<Grade> filtered = grades.stream()
                        .filter(g -> g.getSubject() != null && g.getSubject().getId() == currentSubject.getId())
    ...
    private void handleBack() {
        Stage stage = (Stage) lblSelectedClass.getScene().getWindow();
        stage.close();
    }
}